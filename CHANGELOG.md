...
### Fixed
- "Sync All" on a pair now applies the same paused-mirror recovery as the per-mirror Sync endpoint (added in 1.2.6). When a mirror's batch trigger raises `GitLabMirrorPausedError`, the loop reconfigures the mirror with `enabled=true` (which clears GitLab's hard-failure pause counter) and retries the trigger once. Without this, the per-mirror Sync button worked on a paused mirror but Sync All still surfaced "Mirror is paused on GitLab" errors in the result list. Applies to both pull and push directions.
- "Sync All" on a pair with many mirrors no longer fails with `greenlet_spawn has not been called; can't call await_only() here. Was IO attempted in an unexpected place?` after the first per-mirror error. The batch loop snapshots each mirror's data (id, GitLab mirror id, project ids/paths, token expiration) into plain dicts before iterating, instead of holding live ORM instances. The previous implementation would `await db.rollback()` whenever an iteration's GitLab call or commit failed; SQLAlchemy rollback expires every ORM instance attached to the session (and there is no `expire_on_rollback=False` knob), so on the next iteration the closure passed to `RateLimiter.execute_with_retry` would access `mirror.mirror_id` synchronously, triggering a sync lazy-load on an expired AsyncSession instance and raising the greenlet error.
- Status updates inside the loop are now issued via a bulk `UPDATE Mirror WHERE id = :id` statement rather than mutating an ORM-tracked instance, so a commit failure on one mirror cannot leave the rest of the loop's state corrupted via identity-map expiration.
- Removed the unnecessary outer `await db.rollback()` in the GitLab-error branch (the trigger call happens before any DB write, so there was nothing to roll back — the rollback was the actual cause of the cascading failure for subsequent mirrors).
- Regression test in `tests/test_api_batch_sync.py::test_sync_all_mirrors_continues_after_first_failure` asserts that with 3 mirrors where the first GitLab trigger raises, the next two are still triggered and the response shows `succeeded=2, failed=1` with no greenlet error in the error list.
